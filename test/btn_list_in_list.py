    ...
    $('#btn_list_headers').click(function(e) {
        if(e.target.className.indexOf('ik_list_header') != -1) {
            ...
            notebook.find('.cell h1, .cell h2, .cell h3').each(function() {
                var text_link = $(this).contents().get(0).nodeValue,
                    link = "#"+$(this).prop('id');

                if($(this).tagName.indexOf('h1')  >= 0) {
                    el = $('<li><a class="js-ik_list_item ik_list_item" href="'+link+'">'+text_link+'<ul class="sublist"></ul></a></li>');
                    ul.append(el);  
                    sublist = el.find('.sublist');
                }
                else {
                    sublist.append('<li><a class="js-ik_list_item ik_list_item" href="'+link+'">'+text_link+'</a></li>');    
                }
            })
        }
        else {
